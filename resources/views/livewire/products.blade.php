            ...
            <div class="col-md-2 ">
                            ...
                            @forelse ($categories as $c)
                                <li class="nav-item">
                                    <input id="{{ $c->slug }}" type="checkbox"
                                        wire:model="categoryInputs"value="{{ $c->id }}" id=""><label
                                        for="{{ $c->slug }}"> <span class="mx-2">{{ $c->name }}</span></label>
                                </li>
                            @empty
                                <li class="nav-item">
                            ...
                            @forelse ($brands as $b)
                                <li class="nav-item">
                                    <input id="{{ $b->id }}" type="checkbox"
                                        wire:model="brandInputs"value="{{ $b->id }}" id=""> &nbsp; <label
                ...
                <div class="bg-custom ">
                    <nav class=" bg-custom">
                        ...
                        <ul class="navbar-nav px-4">
                            <li class="nav-item">
                                <input id="hl" type="radio" name="pricesort" wire:model="priceInputs" value="h-to-l"
                                    id=""> &nbsp; <label for="hl">High To Low</label>
                                <br> <input id="lh" type="radio" name="pricesort" wire:model="priceInputs"
                                    value="l-to-h" id=""> &nbsp; <label for="lh">Low To High </label>
                            </li>
                        </ul>
                    </nav>
                </div>
                        ...
                        <div class="col-md-3">
                            <div class="bg-custom card my-1 product-card ">
                                <div class=" card-header text-center fw-bold fs-5 p-0">
                                    ...
                                    @endif
                                    <br>
                                    <a href="{{ url('viewproduct/' . $p->id) }}"class="text-decoration-none   text-white">{{ $p->name }}</a>
                                </div>
                                <div class="card-body"style="height: 300px;  width:100%;">
                                    @if (isset($p->productImages[0]))
                                        <a href="{{ url('viewproduct/' . $p->id) }}">
                                            <img src="{{ url($p->productImages[0]->image) }}" alt="{{ $p->name }}"
                                ...
                                <div class="p-4">
                                    <div class=" text-center my-2 ">
                                        ...
                                        <span  class="text-danger text-decoration-line-through">&#8377; {{ $p->orignal_price }}</span>
                                    ...
                                    <a href="{{ url('viewproduct/' . $p->id) }}" class="btn btn-custom float-end card-link"> 
